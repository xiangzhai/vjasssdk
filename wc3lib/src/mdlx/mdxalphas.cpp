...
MdxAlphas::~MdxAlphas()
{
	BOOST_FOREACH(class MdxAlpha *alpha, this->m_alphas)
		delete alpha;
}
		
std::streamsize MdxAlphas::readMdx(std::istream &istream) throw (class Exception)
{
...
std::streamsize MdxAlphas::writeMdx(std::ostream &ostream) const throw (class Exception)
{
	...
	BOOST_FOREACH(const class MdxAlpha *alpha, this->m_alphas)
		bytes += alpha->writeMdx(ostream);
