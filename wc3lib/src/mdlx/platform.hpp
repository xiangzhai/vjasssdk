...
inline istream& parseMdlNamedBlock(istream &stream, const string &scopeName, string &name, std::streamsize &sizeCounter, bool optional = false) throw (class Exception)
{
	string identifier;
	...
	if (identifier != scopeName)
	{
		if (optional)
		{
			...
			throw Exception(boost::format(_("Scope name \"%1%\" does not match \"%2%\".")) % identifier % scopeName);
	}
	
	parse(stream, identifier, counter);
	
	if (identifier.size() < 2 || identifier[0] != '"' || identifier[identifier.size() - 1] != '"')
		throw Exception(boost::format(_("Identifier \"%1%\" has not the correct format.")) % identifier);
	
	ascii bracket;
	parse(stream, bracket, counter);
	
	if (bracket != '{')
		throw Exception(boost::format(_("\"%1%\" is not a bracket.")) % identifier);
...
inline istream& parseMdlProperty(istream &stream, std::list<struct MdlxPropertyInformation> &properties, std::streamsize &sizeCounter) throw (class Exception)
{
	...
	BOOST_FOREACH(MdlxPropertyInformation &information, properties)
	{
		if (information.identifier == identifier || (information.isStatic && identifier == "static"))
		{
			...
			if (information.type != MdlxPropertyInformation::None)
			{
