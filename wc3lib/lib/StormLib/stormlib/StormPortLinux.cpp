...
HANDLE CreateFile(const char *sFileName, DWORD ulMode, DWORD ulSharing, void *pSecAttrib, DWORD ulCreation, DWORD ulFlags, HANDLE hFile)
{
	...
	switch (ulCreation)
	{
		...
		case OPEN_ALWAYS:
			return (HANDLE)open(sFileName, O_RDWR | O_CREAT, umask(S_IRWXU | S_IRWXG | S_IROTH));

		case CREATE_ALWAYS:
		case CREATE_NEW:
			return (HANDLE)open(sFileName, O_RDWR | O_CREAT | O_TRUNC, umask(S_IRWXU | S_IRWXG | S_IROTH));
